...
if (isset($_POST['submit'])) {
    ...
    $select = "SELECT * FROM login WHERE email = ? AND password = ?";
    ...
    if (mysqli_stmt_num_rows($stmt) > 0) {
        // Valid credentials, redirect to student_info.php
        header('location: student_info.php');
        exit();
    } else {
        // Invalid credentials, set error message for JavaScript to display
        $_SESSION['error'] = 'Incorrect email or password!';
        echo '<script>showErrorMessage("' . $_SESSION['error'] . '");</script>';
    }
...
<!-- ... your existing HTML code ... -->
	...
	<style>
		...
		* {
		...
		body {
			background-image: linear-gradient(rgba(4,9,30,0.7),rgba(4,9,30,0.7)),url(images/beautiful-architecture-office-building-skyscraper-with-window-glass-pattern.jpg);
